  ...
  .addEventListener("click", function (event) {
    ...
    if (input.length === 0) {
      ...
      let currentBalance = parseFloat(
        document.getElementById("noakhaliDonationBalance").innerText
      );
      ...
      document.getElementById("noakhaliDonationBalance").innerText = parseFloat(
        updatedBalance.toFixed(2)
      );

      // Updating AccountBalance
      let accountBalance = parseFloat(
        document.getElementById("acoountBalance").innerText
      );

      accountBalance -= newDonationAmount;
      document.getElementById("acoountBalance").innerText = parseFloat(
